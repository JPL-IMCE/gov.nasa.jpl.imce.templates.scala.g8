  ...
  Project("$name$".replace(".","-"), file("."))
    ...
    .settings(SettingsHelper.makeDeploymentSettings(Universal, packageBin in Universal, "zip"),
      ...
      projectID := {
        ...
        previous.extra(projectSettings(buildUTCDate.value, "$artifact_kind$").toSeq: _*)
      },
...
def projectSettings(buildDate: String, artifactKind: String): Map[String, String] =
{
  return if (artifactKind.isEmpty)
    Map("build.date.utc" -> buildDate)
  else
    Map("build.date.utc" -> buildDate, "artifact.kind" -> artifactKind)
}